...
项目中使用的是 element-ui 的组件库，在使用的时候需要引入 Element 包，具体引入方法请参考 Element 文档 [快速上手](https://element.eleme.cn/#/zh-CN/component/quickstart)
...
以上代码便完成了 DwFormMaking 的引入。需要注意的是，样式文件需要单独引入。
...
/**
  ...
  * success [Function] 成功回调，参数图片地址
  * failure [Function] 失败回调, 参数失败说明
  */
